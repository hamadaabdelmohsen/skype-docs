...
For [UserEndpoint](/dotnet/api/microsoft.rtc.collaboration.userendpoint&preserve-view=true) instances, key publishing APIs cannot be used unless the object referred to by the [LocalOwnerPresence](https://msdn.microsoft.com/library/hh348476\(v=office.16\)) property is in the **Subscribed** state. For [ApplicationEndpoint](/skype-sdk/ucma/user-endpoints-and-application-endpoints#applicationendpoint) instances, it is recommended, but not required, that **LocalOwnerPresence** be in the **Subscribed** state.
